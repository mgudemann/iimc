...
void CSolver::init_parameters(void) {
  ...
  _params.decision.decay_period               = 1; //40;  //AARON
...
CSolver::CSolver(void) {
  ...
  //AARON
  //for(int i = 0; i < 1024; i++) _prefer[i] = 0;
  _did = NULL;
  _didn = 0;
}
...
//AARON 
void CSolver::prefer(int v_idx, int val) {
  assert(val == 0 || val == 1);
  for(int i = 0; i < 1024; i++)
    if (_prefer[i] == 0) {
      _prefer[i] = 2 * v_idx + (1 - val);
      break;
    }
}
...
void CSolver::init_solve(bool full) {
...
#if 1 //AARON
  if (full || variables()->size() > _last_sz) {
    _last_sz = variables()->size();
#endif
  ...
  //AARON set_random_seed(_stats.random_seed);
...
void CSolver::set_var_value_BCP(int v, int value) {
  ...
  for (vector <CLitPoolElement *>::iterator itr = watchs.begin();
    ...
    bool first = true;
    if ((watched+1)->val() <= 0)
      other_watched = watched-1;
    else {
      other_watched = watched+1;
      first = false;
    }
    assert(watched->is_watched());
    assert(other_watched->is_watched());
    assert(literal_value(*watched) == 0);

    CLitPoolElement * ptr = first ? other_watched : watched;
    int the_value = literal_value(*other_watched);
    if (the_value == 0) {
      _conflicts.push_back((watched+(first?1:2))->get_clause_index());
      goto DONE;
    }
    if (the_value != 1) {
      while (true) {
	--ptr;
	if (!ptr->is_literal()) {  // reached end of the clause
          queue_implication(other_watched->s_var(), (watched+(first?1:2))->get_clause_index());
	  break;
	}
	assert(!ptr->is_watched());
	if (literal_value(*ptr) != 0) {
	  int v1 = ptr->var_index();
	  int sign = ptr->var_sign();
	  CLitPoolElement tmp = *watched;
	  *watched = *ptr;
	  *ptr = tmp;
	  variable(v1).watched(sign).push_back(watched);
	  watched->set_watch(1);
	  // remove the original watched literal from watched list
	  ptr->unwatch();
	  *itr = watchs.back();  // copy the last element in its place
...
void CSolver::delete_clause_group(int gid) {
  ...
  //AARON
  //for(int i = 0; i < 1024 && _prefer[i] != 0; i++)
  //  _prefer[i] = 0;
  ...
  if (gid <= 0 || _min_idx < clauses()->size())
  for (vector<CClause>::iterator itr = clauses()->begin() + (gid > 0 ? _min_idx : 0);
       itr <= clauses()->begin() + (gid > 0 ? _max_idx : clauses()->size()-1); 
       ++itr) {
#endif
    CClause & cl = *itr;
...
#if 1
        //AARON
        for (unsigned int i = 0; i < cl.num_lits(); ++i)
          affected.insert(cl.literal(i).var_index());
#endif
...
#else
  //AARON
  for (set<unsigned>::iterator aitr = affected.begin();
         aitr != affected.end(); ++aitr) {
    vector<CVariable>::iterator itr = _variables.begin() + *aitr;
#endif
...
bool CSolver::time_out(void) {
  if (_params.time_limit < 0) return false;
  ...
  */
  ...
  for (unsigned i = _max_score_pos; i < _ordered_vars.size(); ++i) {
	...
	if (_ordered_vars[index].first->value() == UNKNOWN &&
      ...
      if (sign == 0) {
	for(int i = 0; i < 1024; i++) {
	  if (_prefer[i] == 0) break;
	  if (_prefer[i] == 2 * var_idx) {
	    sign = 0;
	    break;
	  }
	  else if (_prefer[i] == 2 * var_idx + 1) {
	    sign = 1;
	    break;
	  }
	}
...
int CSolver::preprocess(void) {
  ...
  for (unsigned i = 1, sz = variables()->size(); i < sz; ++i) {
#else
  for (set<unsigned int>::const_iterator it = _puni_phased.begin(); 
       it != _puni_phased.end(); 
       ++it) {
    unsigned i = *it;
#endif
...
#else
  //AARON
  for (unsigned i = _min_unit_idx, sz = clauses()->size(); i < sz; ++i) {
#endif
  ...
  if (deduce() == CONFLICT) {
    //AARON
    if (_params.verbosity > 1) cout << " CONFLICT during preprocess " <<endl;
#ifdef VERIFY_ON
...
int CSolver::solve(bool full_init) {
  if (_stats.outcome == UNDETERMINED) {
    init_solve(full_init);
...
#define UNMARK 0x7FFFFFFF
#define MCC_MAX 2
bool CSolver::mcc_bounded(int n, int vid, int * gflag) {
  if (n == MCC_MAX) return false;
  ...
  for(CLitPoolElement * itr = clause(ante).literals(); (*itr).val() > 0; ++itr) {
    int v = (*itr).var_index();
    if (v != vid && !variable(v).is_marked() && !mcc_bounded(n+1, v, gflag))
      return false;
  }
  *gflag |= clause(ante).gflag();
  return true;
}
int CSolver::minimize_cc(int gflag) {
  int * lits = &(*_conflict_lits.begin());
  int n_lits = _conflict_lits.size();

  for(int i = 0; i < n_lits; ++i)
    variable(lits[i]>>1).set_marked();

  for(int i = 0; i < n_lits; ++i)
    if (mcc_bounded(0, lits[i]>>1, &gflag))
      variable(lits[i]>>1).clear_marked();

  int j = 0;
  for(int i = 0; i < n_lits; ++i)
    if (variable(lits[i]>>1).is_marked()) {
      variable(lits[i]>>1).clear_marked();
      lits[j++] = lits[i];
    }
    else {
      --_num_in_new_cl;
      variable(lits[i]>>1).set_new_cl_phase(UNKNOWN);
    }
  _conflict_lits.resize(j);

  //if (j < n_lits)
  //  cout << flush << "min " << n_lits << " " << j << endl << flush;
...
int CSolver::finish_add_conf_clause(int gflag) {
  ...
  for (unsigned i = 0; i < clause(added_cl).num_lits(); ++i) {
    ...
    if (dl < dlevel()) {
      ...
      //AARON assert(unit_lit == -1);
#ifdef AARON_EMPOWER
      if (unit_lit != -1) empowered = true;
#endif
...
int CSolver::mem_usage(void) {
  ...
  //Aaron
  int aa = 0;
  aa += sizeof(int) * _recent_shrinkings.size();
  aa += 2 * sizeof(int) * _hooks.capacity();
  aa += 2 * sizeof(int) * _ordered_vars.capacity();
  aa += sizeof(ClauseIdx) * _conflicts.capacity();
  aa += sizeof(int) * _conflict_lits.capacity();
  aa += sizeof(int) * _resolvents.capacity();
  aa += 2 * sizeof(int) * _shrinking_cls.size();

  return aa + mem_dbase + mem_assignment;
}

void CSolver::clean_up_dbase(void) {
  assert(dlevel() == 0);

  int mem_before = _params.verbosity > 0 ? mem_usage() : 0;
  // 1. remove all the learned clauses
  for (vector<CClause>::iterator itr = clauses()->begin();
       itr != clauses()->end() - 1; ++itr) {
    CClause & cl = * itr;
    if (cl.status() == CONFLICT_CL) //AARON cl.status() != ORIGINAL_CL /*AARON*/ && cl.status() != DELETED_CL)
      mark_clause_deleted(cl);
  }
  // delete_unrelevant_clauses() is specialized using berkmin deletion strategy

  // 1.5. AARON, adapted from delete_clause_group
  // delete the index from variables
  for (vector<CVariable>::iterator itr = variables()->begin();
         itr != variables()->end(); ++itr) {
    for (unsigned i = 0; i < 2; ++i) {  // for each phase
  ...
  if (_params.verbosity > 0) {
    ...
    cout << "Database Cleaned, releasing (approximately) "
         << mem_before - mem_after << " Bytes ("
	 << mem_before << " -> " << mem_after << ")" << endl;
  }
}
...
void CSolver::unit_antecedents(vector<int> & lid) {
  ...
  if (!_did) {
    _did = (char *) malloc(_variables.size());
...
bool CSolver::merge(list<unsigned int> & into, const vector<unsigned int> & vec) {
  ...
  if (into.empty()) {
    into.insert(into.begin(), vec.begin(), vec.end());
    return true;
  }
  else {
    bool changed = false;
    list<unsigned int>::iterator it = into.begin();
    vector<unsigned int>::const_iterator jt = vec.begin();
    ...
    while (jt != vec.end()) {
      ...
      if (jt == vec.end()) break;
      while (it != into.end() && *jt >= *it++);
      if (it == into.end()) {
        into.insert(it, jt, vec.end());
        break;
      }
    }
...
void CSolver::lift(int * apvars, int n) {
  set<int> pvars;
  if (apvars)
    for (int i = 0; i < n; ++i)
      pvars.insert(apvars[i]);
  ...
  for (unsigned int i = 0; i < variables()->size(); ++i) {
    ...
    if (var.antecedent() == NULL_CLAUSE &&
        var.watched(0).empty() && 
        var.watched(1).empty() &&
        _puni_phased.find(i) == _puni_phased.end())
      var.set_value(UNKNOWN);
  }

  // B. Greedy
  // 1. get sufficient set of vars
  set<int> vars;
  vector<int> ovars;
  VCMap vcmap;
  int ci = 0;
  for (vector<CClause>::iterator it = clauses()->begin(); it != clauses()->end(); ++it, ++ci) {
    if (it->status() == DELETED_CL) continue;
    bool covered = false;
    unsigned nlits = it->num_lits();
    int nvi = -1, used = -1;
    for (unsigned i = 0; i < nlits; ++i) {
      int vi = it->literal(i).var_index();
      CVariable & var = variable(vi);
      if (var.value() != it->literal(i).var_sign()) {
        if (var.antecedent() != NULL_CLAUSE) {
          used = -1;
          covered = true;
          break;
        }
        else if (vcmap.find(vi) != vcmap.end()) {
          used = vi;
          covered = true;
        }
        else if (nvi == -1 || pvars.find(vi) == pvars.end())
          nvi = vi;
      }
    }
    if (!covered) {
      assert (nvi != -1);
      vector<unsigned int> clss;
      clss.push_back(ci);
      vcmap.insert(VCMap::value_type(nvi, clss));
      vars.insert(nvi);
      ovars.push_back(nvi);
    }
    else if (used != -1) {
      VCMap::iterator vcit = vcmap.find(used);
      assert (vcit != vcmap.end());
      vcit->second.push_back(ci);
    }
  }
  // 2. reverse prune
  list<unsigned int> covered;
  for (int i = ovars.size()-1; i >= 0; --i) {
    int vi = ovars[i];
    VCMap::const_iterator it = vcmap.find(vi);
    assert (it != vcmap.end());
    if (!merge(covered, it->second))
      vars.erase(vars.find(vi));
  }
  // 3. set rest to unknown
  set<int>::const_iterator it = vars.begin();
  for (int i = 0; i < variables()->size(); ++i)
    if (it == vars.end() || i < *it) {
      if (variable(i).antecedent() == NULL_CLAUSE)
        variable(i).set_value(UNKNOWN);
    }
    else
      ++it;
}
