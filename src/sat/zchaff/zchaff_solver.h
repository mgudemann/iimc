...
class CSolver:public CDatabase {
  ...
  protected:
    ...
    bool mcc_bounded(int, int, int *);
    int minimize_cc(int);  //AARON
    ...
    //Aaron
    typedef unordered_map<int, vector<unsigned int> > VCMap;
    bool merge(list<unsigned int> & into, const vector<unsigned int> & vec);
  public:
