...
ClauseIdx CDatabase::get_free_clause_idx(void) {
  ...
  */
  ...
  if (_unused_clause_idx.size() == 0) {
    ...
    set<ClauseIdx>::iterator it = _unused_clause_idx.begin();
    ClauseIdx new_cl = *it;
    _unused_clause_idx.erase(it);
    clause(new_cl).set_id(_stats.num_added_clauses);
    return new_cl;
  }
}
...
ClauseIdx CDatabase::add_clause(int * lits, int n_lits, int gflag) {
  ...
  //AARON
  if (gflag != 0) {
    _min_idx = _min_idx > new_cl ? new_cl : _min_idx;
    _max_idx = _max_idx < new_cl ? new_cl : _max_idx;
  }
  if (n_lits == 1 && new_cl < _min_unit_idx)
    _min_unit_idx = new_cl;
  ...
  if (cl.num_lits() > 1) {
...
#ifdef WLEFT
	CLitPoolElement t = cl.literal(n_lits-1);
	cl.literal(n_lits-1) = cl.literal(i);
	cl.literal(i) = t;
        v.watched(v_sign).push_back(&cl.literal(n_lits-1));
        cl.literal(n_lits-1).set_watch(-1);
#else
...
#ifdef WLEFT
      CLitPoolElement t = cl.literal(n_lits-1);
      cl.literal(n_lits-1) = cl.literal(max_idx);
      cl.literal(max_idx) = t;
      variable(v_idx).watched(v_sign).push_back(&cl.literal(n_lits-1));
      cl.literal(n_lits-1).set_watch(-1);
#else
    ...
    for (i = sz-1; i >= 0; --i) {
      ...
      if (literal_value(cl.literal(i)) != 0) {
#ifdef WLEFT
	CLitPoolElement t = cl.literal(n_lits-2);
	cl.literal(n_lits-2) = cl.literal(i);
	cl.literal(i) = t;
        v.watched(v_sign).push_back(&cl.literal(n_lits-2));
        cl.literal(n_lits-2).set_watch(-1);
#else
        v.watched(v_sign).push_back(&cl.literal(i));
        cl.literal(i).set_watch(-1);
#endif
...
#ifdef WLEFT
      CLitPoolElement t = cl.literal(n_lits-2);
      cl.literal(n_lits-2) = cl.literal(max_idx);
      cl.literal(max_idx) = t;
      variable(v_idx).watched(v_sign).push_back(&cl.literal(n_lits-2));
      cl.literal(n_lits-2).set_watch(-1);
#else
      variable(v_idx).watched(v_sign).push_back(&cl.literal(max_idx));
      cl.literal(max_idx).set_watch(-1);
  ...
  //AARON
  if (gflag == 0) {
    for (unsigned i = 0, sz = n_lits; i < sz; ++i) {
      unsigned v_idx = cl.literal(i).var_index();
      if (variable(v_idx).lits_count(0) != 0 &&
          variable(v_idx).lits_count(1) != 0) {
        set<unsigned>::iterator it = _puni_phased.find(v_idx);
        if (it != _puni_phased.end())
          _puni_phased.erase(it);
      }
    }
  }
